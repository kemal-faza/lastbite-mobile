...
export default function CheckoutScreen() {
  ...
  const items = storeName ? filterForCheckout(allItems, storeName) : allItems;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      router.replace(
        `/orders/${res.order.id}?justChecked=true`,
      );
    } catch (e: any) {
    ...
    <View className="flex-1 bg-background">
      ...
      >
        ...
        {items.map((item) => (
          <View
            key={item.id}
            className="bg-white p-3 rounded-xl mb-2 flex-row justify-between items-center"
          >
        ...
        <View className="bg-secondary/10 p-3 rounded-lg mb-4">
          <Text className="text-sm text-gray-600">
            Pembayaran dilakukan saat mengambil pesanan (COD). Platform tidak
